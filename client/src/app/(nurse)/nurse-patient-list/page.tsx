...
function PatientList() {
	...
	const fetchPatientsA = async () => {
		...
		if (fetchDoctors.items.length === 0) {
			toast({
				title:
					"Debe registrar un especialista antes de registrar a los pacientes",
				status: "error",
				duration: 5000,
				isClosable: true,
			});
				...
				>
					{doctorExist ? (
						<Link href={routes.nurseRegisterPatient}>
							<Text color="#033e5c">+ Registrar paciente nuevo</Text>
						</Link>
					) : (
						<span>
							...
							<TabPanel>
								...
								{!(patientsA.length > 0) && (
									<Flex justifyContent="center" marginTop={4}>
										<Text color="#033e5c">No hay pacientes activos</Text>
									</Flex>
								)}
							</TabPanel>
							<TabPanel>
								...
								{!(patientsI.length > 0) && (
									<Flex justifyContent="center" marginTop={4}>
										<Text color="#033e5c">No hay pacientes inactivos</Text>
									</Flex>
								)}
							</TabPanel>
						</TabPanels>
