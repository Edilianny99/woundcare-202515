...
const ModalComponent: React.FC<ModalComponentProps> = ({
	...
	const handleSubmit = async () => {
		if (type === "Peso y altura") {
			...
			if (
				cellPhoneNumber.length > 16 ||
				cellPhoneNumber.length < 9 ||
				phoneNumber.length > 11 ||
				phoneNumber.length < 7 ||
				cellPhoneNumber === "" ||
				phoneNumber === "" ||
				isNaN(Number(cellPhoneNumber)) ||
	...
	return (
		<Modal isOpen={isOpen} onClose={onClose}>
			<ModalOverlay />
			<ModalContent
				sx={{ backgroundColor: "#fff", maxW: "90vw", margin: "auto" }}
			>
				<ModalHeader>{type}</ModalHeader>
				<ModalBody>
					<Box w="100%" h="2px" bg="#033e5c" mt="-10px" mb="10px" />
