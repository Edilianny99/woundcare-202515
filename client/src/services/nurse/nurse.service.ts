...
export const getDoctors = async (
	...
	const data = await fetchAPI<PaginatedResponse<Doctor>>(
		`/doctor?${page ? `page=${page}&` : ""}${
...
export const getPatientsInactive = async (
	page?: number,
	perPage?: number
): Promise<PaginatedResponse<Patients>> => {
	const data = await fetchAPI<PaginatedResponse<Patients>>(
		`/patient/inactive/nurse?${page ? `page=${page}&` : ""}${
			perPage ? `per-page=${perPage}` : ""
		}`,
		"GET"
	);
...
export const dischargePatient = async (nationalId: string) => {
	const data = await fetchAPI(
		`/medical-file/patient/${nationalId}/discharge`,
		"PATCH"
	);
	return data;
};
