...
export const createAllergies = async (name: string) => {
	const data = await fetchAPI<Patient>("/patient/me/allergies", "POST", {
		name,
	});
	return data;
};
...
export const editWeight_Height = async (weight: number, height: number) => {
	const data = await fetchAPI<Patient>("/patient/me", "PATCH", {
		weight,
		height,
	});
	return data;
...
export const createMedicalRecords = async (description: string) => {
	const data = await fetchAPI<Patient>("/patient/me/medical-records", "POST", {
		description,
	});
	return data;
};
...
export const deleteMedicalRecords = async (description: string) => {
	const data = await fetchAPI<Patient>(
		`/patient/me/medical-records/${description}`,
		"DELETE",
		{ description }
	);
	return data;
...
export const editCellPhoneNumber = async (
...
export const getMedicalFileById = async (id: string) => {
	const data = await fetchAPI<TheMedicalFile>(`/medical-file/${id}`, "GET");
	return data;
};
